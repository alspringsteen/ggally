...
if(!identical(plotObj$axisLabels,"internal")) {
  ...
  pushViewport(viewport(width=unit(1, "npc") - unit(6,"lines"), height=unit(1, "npc") - unit(3, "lines")))

  pushViewport(viewport(layout = grid.layout(numCol, numCol, widths = rep(1,numCol), heights = rep(1,numCol) )))

  # Left Side
  for(i in 1:numCol){
    grid.text(names(plotObj$data[,plotObj$columns])[i],-0.05,0.5,rot=90,just=c("centre","centre"), gp = gpar(fontfamily="Helvetica",fontsize=9,col="black"), vp = vplayout(as.numeric(i),1))
  }
  ...
  pushViewport(viewport(width=unit(1, "npc") - unit(3,"lines"), height=unit(1, "npc") - unit(6, "lines")))
  ...
  # Bottom Side
  for(i in 1:numCol){
    grid.text(names(plotObj$data[,plotObj$columns])[i],0.5,-0.05,just=c("centre","centre"), gp = gpar(fontfamily="Helvetica",fontsize=9,col="black"), vp = vplayout(numCol, i))
  }

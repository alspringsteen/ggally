...
ggally_points <- function(data, mapping, ...){
  ...
  n=7 # we will grab the middle 3 break points
  ...
  if(dif > 50 & dif < 100) tiks <- mround(breaks[c(2,4,6)], 10)
...
ggally_cor <- function(data, mapping, corAlignPercent = 0.6, ...){
  ...
  if(colorCol != "ggally_NO_EXIST" && colorCol %in% colnames(data)) {
    ...
    theme(legend.position = "none", 
          ...
          panel.border = element_rect(linetype = 1, colour = "black", fill=NA)) #dashed
